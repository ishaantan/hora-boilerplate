---
name: hn-spec-backend
description: Stage 4 of /hn-spec. Declare the repositories and servers, then design the data model, the operation list and the background jobs that every stage-1 use case can be walked against. Invoked by /hn-spec, or directly.
---

# hn-spec-backend

**Stage 4 of `/hn-spec`.** Design what holds the data, what operates on it, and what runs outside the request — and prove, use case by use case, that the result can serve them.

Read `../hn/references/structure.md` and `../hn-spec/references/principles.md` first. `../hn-spec/references/stages.md` is the authority on this stage's exit condition, `../hn/references/spec-format.md` on the format of every table written here, and `../hn/references/asking.md` on how anything is put to a person.
...
**The whole existing data model and operation list goes out as checks, batched per area** — one for the tables, one for the operations, one for what runs outside the request. A person confirming the fourteenth table in a row has stopped reading (`../hn-spec/references/investigation.md`).

**A feature listed under `Baseline: inventoried` still owes its tables and its operations a row each here.** The section itself carries a name and one line (`../hn/references/spec-format.md`, "`baseline`") — but its tables are in the database everything designed here has to fit around, and its operations sit on a server stage 6 has to state a caller for. It costs almost no exchanges, since those rows sit inside the batched checks above.
...
| a class name, a table name, a column name that lint has to accept | **`/hn-plan`**, against the lint rules |
| the migration, the model, the resolver, the job class | **`/hn-build`**, at checkpoints 2 to 7 |
...
**Declared, never assumed** — `/hn-setup` reads this table and stops without it.
...
- **A repository that already existed** gets the optional `Directory` column, and `/hn-setup` then never clones it

Everything else about this table is in `../hn/references/spec-format.md`.

## 2. Roles on one endpoint, or endpoints of their own

**The most consequential choice this stage makes.** `../hn-spec/references/principles.md` holds the full decision table; the three cases in one line each:
...
- **Every table names which use cases it serves** — **except a table belonging to a listed feature, which names that feature instead**, since a listed section states no use case (`../hn/references/spec-format.md`, "`baseline`")
...
- **Do not name the model class or the migration.** `/hn-plan` decides identifiers against the lint rules

## 4. The operations

**The stack handbook's default API style is what a silent spec has chosen; another style needs a stated reason** (`../hn-spec/references/principles.md`).
...
**Every operation states its kind, and the kind is never inferred.** `/hn-build` branches on the value at three separate checkpoints. Ask, and write what was said.
...
| `../hn-spec/references/principles.md` | roles or endpoints, the default API style or another, synchronous or a job, scale as a number |
| `../hn/references/spec-format.md` | the format of every table here, and what stops the run |
| `../hn-build/references/checkpoints.md` | checkpoints 2 to 7, which build what this stage designs |
