---
name: hn-spec-nonfunctional
description: Stage 3 of /hn-spec. Turn "it should be fast" into the four numbers that change a design — users now, users foreseen, the heaviest operation, and how long data is kept — plus availability, the security level and the middleware. Invoked by /hn-spec, or directly.
---

# hn-spec-nonfunctional

**Stage 3 of `/hn-spec`.** Get the numbers that decide stage 4's design, and the middleware `/hn-setup` needs in order to bring anything up at all.
...
**Offer numbers as options, never as a blank.** "How many users?" produces a shrug; `200 / 2,000 / 20,000`, with today's row count named alongside, produces an answer in one exchange (`../hn/references/asking.md`).
...
**This stage produces constraints, not designs.** A number written here becomes a constraint on every feature (`../hn/references/spec-format.md`), and the design it forces is stage 4's to make.
...
**And the middleware, which is what `/hn-setup` reads:**
...
- **Write the server's version, not a driver's.** An npm dependency does not indicate the server version, and without it `/hn` has to guess
...
- **`profile` and `COMPOSE_PROFILES` are what `/hn-setup` derives from this table** — a middleware the table omits is one that never comes up, and acceptance stops rather than reviewing something that is not really running

---

## Exit condition

The four numbers written, as numbers or as stated assumptions; availability, retention and the security level written; every middleware declared with its server version. `../hn-spec/references/stages.md` is the authority.
...
| `../hn-spec/references/principles.md` | "Scale is a number, or it is nothing" |
| `../hn/references/spec-format.md` | "Non-functional requirements", "Manual verification" |
| `/hn-setup`, as the project's boilerplate ships it | what it does with the middleware table |
