---
name: hn-build
description: Build one feature by taking it through the seventeen checkpoints — spec, backend, frontend, acceptance — in order, one gate at a time. Runs at the root of the hora repository, one feature per invocation. Invoked by /hn, or directly as /hn-build.
---

# hn-build

**Take one feature through the seventeen checkpoints, in order.**

Read `../hn/references/structure.md` (the layout, the invariants, where a command runs) and `references/checkpoints.md` (the checkpoint list itself) before starting. **This skill is read-only on `specs/`; checkpoint 1 is where a problem found there is routed to the two skills that may write it** (`references/checkpoints.md`, checkpoint 1).

## One feature at a time, never two

**Nothing here ever runs alongside another feature's checkpoints.** One feature goes from checkpoint 1 to checkpoint 17, and only then does the next one start:
...
| a break shows up in the run that caused it | checkpoint 17's unit suites cover every feature so far, so a regression fails immediately |
...
Report the decision in one line before starting work — "building #attendance, from checkpoint 5 of 18".

**A feature whose only open checkpoint is 18 writes no code, and cuts no branch.** That is what a section declaring `<!-- built: frontend -->` looks like on adoption: everything up to the acceptance gate is marked not-applicable, so this skill goes straight to `/hn-accept`.

**Step 2 skips an entry whose checkpoint 17 is covered by the sweep entry.** Where every specified feature carries `built:`, `/hn-plan` collapses the per-feature gates into a single adoption sweep (`../hn-plan/SKILL.md`, "collapses to one sweep"). Taking one anyway would run the twenty individual gates the collapse exists to replace.

**A listed feature is never entered, and it has nothing to resume from.** A section carrying `<!-- baseline: inventoried -->` sits under `## Not accepted` with no checkbox, so step 2 skips it with no special case (`../hn-plan/SKILL.md`, "`_plan.md` — the order"). Its seventeen empty boxes are not a feature nobody started — the feature file's provenance header says which of the two this is, and opening one rebuilds code that is already serving users.
...
**A listed feature is not one of the unfinished ones.** It carries no checkbox, so a version with nothing left but listed entries is finished rather than deadlocked (`../hn/SKILL.md`, "Deciding where you are").
...
5. Run it:
     ...
     an auditing checkpoint (8)
       -> hn-verifier, read-only, given the skill names to invoke in full
...
9. Verify the exit condition actually holds — with hn-verifier for anything
...
**Step 10's split matters.** The checkbox is written the moment the checkpoint passes, so an interrupted run resumes at the right place; the commit happens once per gate, so `git log .hora/` stays readable (`../hn/references/commits.md`, "Committing `.hora/`").

### Step 3 — matching a checkpoint to the skills that cover it

**`references/checkpoints.md` names no package skill, and it never may** (`../hn/references/structure.md`, "No hora file ever names one of those skills"). So the match is made here, once per checkpoint, against what is actually equipped:
...
5. Write the names, and the content hash each digest was derived at, into
   the feature file against this checkpoint
5. Hand the names and the digest paths to the agent, in its assignment
```

```markdown
- [x] 14. UI  <!-- skills: <every name you matched, comma-separated>; digests: <each skill's content hash the digest was derived at> -->
```
...
A checkpoint line carries a second comment at its end, holding what running the checkpoint cost. The first comment stays exactly as it is: `/hn-plan` reads `<!-- n/a: … -->` by its text.

```markdown
- [x] 5. Actual API  <!-- skills: …; digests: … --> <!-- cleared: 1; reopened-by: 9; agents: 4; agent-time: 840s; verify-time: 240s; wall-time: 1310s -->
```
...
| `agents:` | how many hn-implementer and hn-verifier agents this line started, over every run |
| `agent-time:` | seconds of wall clock from starting them to the last one returning, over every run |
| `verify-time:` | the hn-verifier share of `agent-time:` |
| `wall-time:` | seconds of wall clock from entering the checkpoint to writing its box, or to stopping short, over every run. On 1, 8 and 10 it is a verifier's pass, plus a person only where one found something |
| `agent-tokens:`, `verify-tokens:` | the same split, only where the Agent tool reports tokens |

**Add this run to it whether the checkpoint passed or not** — the moment step 9 has judged, or the moment the feature stops short of it. A checkpoint settled in conversation carries only `cleared:`, `reopened-by:` and `wall-time:`. The record survives whoever rewrites the line, and a clear adds to it (`../hn/references/structure.md`, "What lives in `.hora/`").
...
3. For the rest, start hn-digester — one agent per skill, all in one
...
**The content hash in the header is what keeps a digest honest.** A digest holds only while the hash it names still matches `kit/skills/<skill-name>/` on disk, so editing one skill leaves that skill's digest to be rewritten before it is read again — and leaves every other digest alone. **An uncommitted edit moves the hash too**, which is the case a version number could never have caught.

**A digest names the skill it came from, and the agent reads that skill whenever a question stays open** (`../../agents/hn-digester.md`), so a convention a digest states too thinly costs one read.
...
**Checkpoint 7's audit skills are invoked in full, and a digest has no part in it.** A step whose skill *is* the criteria runs that skill whole (`../hn/references/structure.md`, "How the match is made").
...
**The exit condition stays whole.** A unit is a slice of the work, never a slice of the gate: step 9 verifies the checkpoint's own condition once, across everything the units produced, and one checkbox covers all of them. **Where the condition asks for something no single unit can see** — checkpoint 4's confirmation that every module checkpoint 5 imports resolves — the main session gathers it at step 6.
...
What hn-verifier is handed at checkpoint 7, and at step 9 wherever it runs: this feature's change set, derived from the repository, never from what an implementer reported. From inside this checkpoint's repository:
...
### Step 9 — when the suite is the verification (checkpoints 5 and 16)
...
2. Did step 8's fix loop touch any test file?
     ...
     yes -> spawn hn-verifier after all, scoped to exactly the test files
...
**1, 2, 9, 11 — the ones that talk to a person.** An agent cannot ask anyone anything. Handing checkpoint 1 to an agent turns "settle this with the author" into "the agent decided", which is invariant 2.
...
`hn-implementer` writes code and tests, for **the one checkpoint — or the one unit of it — that it was handed**, and nothing else. It never touches git, never writes `.hora/`, never writes `specs/`, never installs a dependency, and never edits a file outside its own scope.
...
`hn-verifier` returns a judgment, never a fix (`../../agents/hn-verifier.md`, "What to return").
...
| `findings` (checkpoint 7) | an implementer fixes them, then the audit runs again — **scoped to the fix, never a fresh full re-scan**: confirm each prior finding is resolved, and re-audit the files the fix reported touching (the same set step 7 lints and step 8 tests), **together with any shared surface that fix reached** — a contract caller it rewired, a guard it moved — since those can carry a new finding into a file the fix did not itself edit. An accepted finding is recorded as a question, never left as a silent pass |
| `contractDrift` | raises a `contradiction` question (`blocking: yes`). **Never edits the contract** |
| `specIssues` | takes it to checkpoint 1's procedure, or raises a question |
...
**How a file is named and how imports are ordered are the package's conventions, and neither is restated here** (`../hn/references/structure.md`, "The division of labor"). At step 3, match the equipped skills whose descriptions cover naming and import order, and hand them to the agent.
 ...
 *
 * Code generated by /hn. DO NOT EDIT.
 */
...
**Report the change needed; do not make it yourself.** State what the file needs, under `conflictProof`. This skill applies it on an `update/` branch and commits it there (`../hn/references/commits.md`, "Commit messages").
...
Checkpoint 17 passing is what finishes a feature. Then:
...
**Name and link every question, never count them** (`../hn/references/structure.md`, "Citing a question in a report"). A feature's run can raise a `reinvention`, a `spec-assumption` and an `eslint-exception` without ever stopping.
...
**That same rule is why a listed feature's entry carries no checkbox at all rather than a set one.** All seventeen of its checkpoints are `[ ]`, so `[x]` would claim a pass nothing earned, and `[ ]` would put it in the queue step 2 takes its next feature from (`../hn-plan/SKILL.md`, "`_plan.md` — the order").
...
| File | Content |
|---|---|
| `references/checkpoints.md` | **the seventeen checkpoints** — order, exit conditions, delegates, when each does not apply |
| `../hn/references/structure.md` | the layout, the invariants, where a command runs, the division of labor |
| `../hn/references/commits.md` | branches, commit granularity, merging, hotfix catch-up |
| `../hn/references/done-criteria.md` | what "done" means for a checkpoint, a feature and a version |
| `../../agents/hn-implementer.md` | writes code and tests for one checkpoint, or for one unit of one |
| `../../agents/hn-verifier.md` | adversarially checks one checkpoint's exit condition. Read-only |
| `../../agents/hn-digester.md` | writes one equipped skill's digest into `.hora/digests/` |
