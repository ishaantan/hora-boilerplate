---
name: hn-spec-frontend
description: Stage 5 of /hn-spec. Design the screens each use case passes through and the operations each screen calls, so nothing on a screen lacks an operation and no operation is unreachable — the empty, failed, waiting and forbidden states included. Invoked by /hn-spec, or directly.
---

# hn-spec-frontend

**Stage 5 of `/hn-spec`.** Turn the use cases into screens, and connect every screen to the operations stage 4 designed — in both directions.

Read `../hn/references/structure.md` and `../hn-spec/references/principles.md` first. `../hn-spec/references/stages.md` is the authority on this stage's exit condition, and `../hn/references/asking.md` on how anything is put to a person.
...
| a component, a token, a CSS class, a component file | **`/hn-build`**, at checkpoints 11 to 16 |
| anything about how a screen is built | the package's own frontend skills |

**A screen list is not a design.** What makes this stage worth a gate is the mapping — which use case passes through which screens, and which operations each screen calls. Without it, checkpoint 10 has nothing to verify against.
...
**Invoke what you matched; do not summarize it here.** That context file is read again at checkpoints 10 and 18, so this stage is the cheapest place for it to be filled in correctly — the users, the scope and the project's own UX rules are already settled.
...
**The `Calls` table is what makes the mapping checkable.** Checkpoint 10 reads it, the end-to-end test specification derives scenarios from it, and the acceptance review uses it to find the operation nothing reaches.
...
**Who reaches it is on the line and what it calls is not.** Stage 6 has to state what everybody else sees on that screen, and nothing else in the document names its actor; the `Calls` mapping exists for checkpoint 10, and no checkpoint 10 ever runs for a listed feature. **A screen written out in full here is the stop moved somewhere the stop does not look** (`../hn/references/spec-format.md`, "`baseline`").

---

## Exit condition

Every use case naming its screens in order; every screen naming its operations; nothing unreachable in either direction; every screen's empty, loading, failed and forbidden states written; every `<!-- acceptance -->` block this stage wrote checkable at its own feature's gate, with anything reaching further handed to stage 2 — **every one of those read over the screens this version specifies, a listed feature excepted, which exits with its inherited screens as one line each and nothing else**. `../hn-spec/references/stages.md` is the authority.
...
| `../hn/references/spec-format.md` | the format of a feature section, and its two required blocks |
| `../hn-build/references/checkpoints.md` | checkpoints 10 to 16, which build what this stage designs |
