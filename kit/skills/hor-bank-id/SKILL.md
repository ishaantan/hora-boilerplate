...
An orchestrator runs this, unconditionally, as its very first action against the backend row on any invocation; in Hora Kit that is `/hn-build`. A human recovering from the failure above runs the same command by hand.
