---
name: hn-plan
description: Plan one version from its spec. Fixes which version is being built, verifies the spec for holes and contradictions in conversation, and writes the feature-level task list. Runs at the root of the hora repository. Invoked by /hn, or directly as /hn-plan.
---
...
Read `../hn/references/structure.md` first — the layout, the invariants and the language rule. **`../hn/references/asking.md` fixes how anything here is put to a person**, and it is the same file `/hn-spec` reads.
...
**The task list is feature-level, never implementation-level.** "Build the attendance feature" is an entry. "Write the `RpaFlow` model" is a checkpoint inside `/hn-build`, and this skill does not decide it.

## This skill may write into `specs/`. Only `/hn-spec` may too

Planning is a conversation with whoever wrote the spec, and asking that person to hand-edit twenty separate holes defeats the point of having it. So this skill may write into `specs/`, under `../hn/references/structure.md`, invariant 1:
...
**Step 2 is a proposal, and it is said as one** — "I suggest this edit; it is yours to decide". **Where the finding is instead that this skill may have misread the document, that is a check** — "I read this as X; is that right?" — settled before any edit is proposed (`../hn/references/asking.md`). Half of what looks like a hole in a spec is a hole in the reading of it.
...
**A finding that needs design work goes to `/hn-spec` instead, at the stage that owns it.** The split is by what the fix is, not by how large it looks:
...
| a missing use case, a use case the design cannot serve, an operation with no kind or no caller, a scope split nobody has made, a contradiction between two designed things | **`/hn-spec`**, at the stage `../hn-spec/references/stages.md` names |
...
**`specs/<version>/request/` is the one directory this does not apply to.** `/hn-spec` reads it and drafts sections from it, and **what it produced is in `spec.md` by the time planning starts**. Reading it here would extract tasks from a wish list nobody was held to.
...
|---|---|---|
| **Use cases per feature** — except a section carrying `<!-- baseline: inventoried -->` | checkpoints 1, 8 and 11 have nothing to verify against | **yes** |
| **Acceptance criteria per feature** — the same exception, and only those two | "what counts as done" would have to be invented | **yes** |
| **The kind of each API operation** — query / mutation / subscription / REST renderer | checkpoints 2, 5 and 14 cannot choose which convention to follow | **yes** |
...
| **An order that puts every feature after the features it depends on** | `/hn-build` silently builds them in a different order than the document states | **yes** |
...
| | States | Verified at |
|---|---|---|
| a **use case** | who does what, for what purpose, end to end | checkpoints 1 (does the spec support it), 9 (does the built API support it), 11 (does the screen support it), 18 (does the product support it) |
| an **acceptance criterion** | an observable behavior that is either present or absent | the tests written alongside the code, and checkpoint 17 |
...
**Every gate that reads a feature's blocks runs at that feature's own position in the order**, so a criterion or a use case naming a feature built afterwards cannot be met wherever it is read (`../hn/references/spec-format.md`, "A criterion is checked at its own feature's gate"). **Four runs act on one anyway**: checkpoint 1 builds from the criteria, 6 and 16 write a test for each one and run it, `hn-verifier` reports the untestable one as `missingTests`, and 18 fails the feature by construction.
...
**Batch the deciding, not the approving.** Which findings are real, and which of several fixes to take, go out through the question tool four at a time; the exact wording of each edit is then shown and approved on its own (`../hn/references/asking.md`).
...
- **If even one `blocking: yes` is unresolved, `/hn-build` is not entered.** With only `no` left, warn and continue
...
| `forward-reference` | a feature's use case or acceptance criterion reaches a feature built after it, or the written order contradicts a `depends`. **Fixed at `/hn-spec`, stage 2 — never here** | yes |
...
| `spec-proposal` | an improvement `/hn-spec` proposed and whoever decided declined or deferred it. **Recorded so it is not proposed again every run** | no |
...
| `hotfix-debt` | a `/hn-hotfix` run shipped a fix to `main` without the acceptance review, and that debt is still open | no, but **fail-loud** |
...
**Every operation's kind belongs in the contract, not only its shape.** The contract is where checkpoints 2, 5 and 14 each read it from.
...
**Acceptance appears twice, and the two are different tasks.** Every feature carries its own acceptance as checkpoint 17, covering everything implemented so far; the `## Acceptance` entry is the whole-version sweep that runs once, before the merge into main. Write both.
...
**The count going up is worth reading.** Nine version criteria against eleven features says most of this version's verification has moved to a single run at the end, which is the shape the feature-at-a-time design exists to avoid (`../hn-build/SKILL.md`, "One feature at a time, never two").
...
**No checkbox, for the same reason `## Withdrawn` has none.** A checkbox would have to mean something, and both meanings are wrong: `[ ]` puts a feature nobody intends to build in front of `/hn-build`, and `[x]` claims a pass over seventeen checkpoints not one of which was ever marked.
...
**Nothing in the section is declared. All of it follows from two lines in `specs/`** — `Baseline: inventoried` in `Existing assets`, and the per-feature annotation (`../hn/references/structure.md`, "Where a lever lives"). Delete the section and the next run rebuilds it identically; hand-edit it and the next run overwrites it.

### A version whose every specified feature carries `built:` collapses to one sweep

**The normal shape of an `as-built` adoption is twenty features with checkpoints 1–16 not applicable and 18 open** — twenty per-feature acceptance runs over an ever-growing cumulative scope, each finding mostly what the one before it found. The per-feature gate exists to catch a feature breaking its predecessors **while the change is one commit old**; here nothing is changing.
...
| **listed — `built:` and `<!-- baseline: inventoried -->`** | **does not qualify, and never had a gate.** It keeps its `## Not accepted` entry, its absent checkbox and its seventeen `[ ]` |
...
- [ ] Sweep the whole version — the adoption sweep. Covers checkpoint 17 of every entry above
...
**Every entry under that heading stays `[ ]` until the adoption sweep passes, and then they are set together.** An entry is `[x]` only once every checkpoint of that feature is, and checkpoint 17 always stays `[ ]` here whatever `built:` says. **`_plan.md` derives its checkboxes from the checkpoints; it does not announce results ahead of them.**

**This skill is what sets them, and it is a reconciliation row like every other one in section 6.** The trigger is a state, not an invocation: `.hora/acceptance/<version>/_sweep.md` exists and its **newest block** reads a passing verdict, and entries under the collapsed heading still stand `[ ]`. On finding it, **set checkpoint 17 in each of those features' files and their entries in `_plan.md` in the same write, off that one record** — 18 first, so no entry ever claims more than its own file does.

```markdown
- [x] 17. Acceptance (E2E and unit both)  <!-- the adoption sweep: .hora/acceptance/1.0.0/_sweep.md -->
```

**The writer has to be named here, because neither skill a reader would expect can do it.** `/hn-accept` writes acceptance records and never `_plan.md`; `/hn-build`'s own "set the feature's entry to `[x]`" step never fires, because step 2 of its "Where to start" skips exactly the entries a sweep entry covers. With no writer named, twenty entries stand `[ ]` over a sweep that passed, and the version can never be finished.

**It stays a derivation, and that is why it waits for the record rather than for the run.** A pass the record does not carry sets nothing, however certain the run that produced it was — and since `/hn` enters this skill on every invocation, the record and the boxes are never more than one invocation out of step.
...
|---|---|
| whatever selects the next feature to build | **not a candidate.** Its checkpoint 17 is covered by the `## Acceptance` entry (`../hn-build/SKILL.md`, "Where to start") |
| the acceptance sweep, deciding its scope | **in scope.** A collapsed version's sweep covers every entry under the heading, whatever its box reads (`../hn-accept/SKILL.md`, "What is in scope") |
...
**Checkpoint 17 always stays `[ ]`.** No reading of an existing repository can stand in for an acceptance review.

**A listed feature gets the same file, and not one line of it is dropped.** What changes is the header and the marks: two non-checkbox lines say what the listing recorded, and all seventeen checkpoints stay `[ ]`.
...
**Eighteen `[ ]`, and not one of them marked not applicable.** Not `[x]`, and not `n/a` either — a listed feature's checkpoints are marked as *nothing at all* (`../hn/references/spec-format.md`, "`baseline`").

**The header is what stops seventeen empty boxes being read as "never started".** A feature sitting at checkpoint 1 with a screen already in production invites exactly one action: build it. The header says the code exists, says nothing about it has been verified, and says which version listed it.
...
**It is not a constraint, and it does not come off when the dependent passes.** A `Constraint:` line tells an implementer what to leave room for; `Rests on:` tells whoever reads an acceptance record what that pass did not cover. It stays until the debt is paid and the dependent's own checkpoint 17 has been re-earned.
...
**Write every checkpoint, including the ones that will obviously not apply.** A checkpoint this skill leaves out is indistinguishable from one that was forgotten. `../hn-build/references/checkpoints.md` is the authority on the list and its wording — copy it from there, do not paraphrase it.
...
|---|---|
| an aggregation file | from how registration works, seen by `/hn-setup`. Nothing overlaps if scanning is automatic |
...
**A mark here is about two features, and the concurrent case lives elsewhere.** Inside one checkpoint, a file two units would both write is assigned to one of them there (`../hn-build/SKILL.md`, "Step 5 — splitting a checkpoint into units").
...
**This skill runs every time `/hn` runs.** Skip it and sections added to `specs/` after the list was settled are never read at all.

Reconcile the set of sections in the resolved document against the feature files in `.hora/tasks/<version>/`.

| State | Action |
|---|---|
| a section with no feature file | create one. **Append it to `_plan.md`'s end** (do not disturb the existing order). **One carrying `<!-- baseline: inventoried -->` is appended to `## Not accepted` instead, with no checkbox**, and its file gets the provenance header and all seventeen `[ ]` (section 5) |
...
| a collapsed version whose `_sweep.md` has a newest block reading a pass, over entries still standing `[ ]` | **set checkpoint 17 in each of those features' files and their entries under `## Features — adopted as built`, off that one block** (section 5). Nothing else sets them |
| the implementation scope carries a `Reconsider <version>'s scope when:` line naming the version being planned, whose condition now holds | **raise it once, in conversation, as a proposal to re-run stage 2** — naming the condition, what in the plan satisfies it, and the `scope` question that recorded the original decline — and **record the outcome as a question naming the line**: declined lands as `spec-proposal` (`blocking: no`, the category that exists so a declined proposal is not re-raised every run); taken hands the run to `/hn-spec` at stage 2. **The recorded question is the record that it fired** — the walk raises nothing where one already names this line |
...
**Which checkpoints a spec change invalidates depends on what changed, and this skill decides it, not `/hn-build`.**

| What changed in the section | Clear from |
|---|---|
| a use case | checkpoint 1 — everything after it |
| the data model, or an API's shape or kind | checkpoint 2 |
| an acceptance criterion only | checkpoint 17 |
| a screen or an interaction only | checkpoint 10 |
| **the version's own acceptance criteria** | **the `## Acceptance` sweep entry alone — not one feature's checkpoint 17** |
...
**When it cannot be told apart, clear from checkpoint 1.** Rebuilding more than was necessary costs time; leaving a checkpoint marked passed against a spec it no longer satisfies costs correctness.
...
**A split needs no reconciliation rules of its own.** The `kicked:` row above moves its entries, the `Version acceptance criteria` digest row re-derives the sweep, and the removal rule covers an implemented mover. Build nothing new for it (`../hn-spec-horizon/SKILL.md`, "Splitting a version under way").

**A section that gains `baseline: inventoried` almost always arrives with checkpoints already marked — up to seventeen of them — and every one of those comes off.** A not-applicable mark is cleared the moment its reason stops holding (`../hn-build/references/checkpoints.md`). The reason here was `built:` expanded into marks, and listing makes `built:` a value recorded and acted on nowhere.
...
**Paying it is a version's ordinary work — and it is the one reconciliation that refuses to act on what the document already says.** The single fact the listing recorded, `built:`, was recorded precisely so that nothing would act on it, and acting on it now would mark up to seventeen checkpoints not applicable on the strength of a value nobody has confirmed since the day it was written. So restate the value, have it confirmed through the question tool with the evidence laid out (`../hn/references/asking.md`), and only then expand it into not-applicable marks.

**Or that version declares `authority: to-spec` for the feature, and all seventeen run against the existing code.** The two never appear on one feature.

**Inside the paid feature there is nothing to clear.** No checkpoint of it was ever marked. And its checkpoint 17, when it comes, is that feature's first acceptance ever: `/hn-accept` decides that run's reach itself, from the absence of any passing record together with the earlier version's `## Not accepted` entry naming it.

**What the payment clears lands on other features: checkpoint 17 of every feature that reaches the paid one transitively — through the same union `Rests on:` was derived from, never through `depends` alone** (section 5). Each of them passed acceptance while resting on behavior nobody had stated, and stating that behavior changes what the pass was measured against.

**A dependent that is itself listed is not in that set, and the exclusion belongs here, where the set is defined.** Re-schedule it and it gets a fresh `## Features` entry with a `[ ]` box, sixteen not-applicable marks and a scheduled acceptance over a feature with no use cases and no acceptance criteria — every state the listing denies. **It never passed anything, so there is nothing to clear and nothing to re-earn**: it keeps its `## Not accepted` entry and its seventeen `[ ]` until its own debt is paid.
...
**Where that clearing lands is the paying version's own plan, and nowhere else.** A dependent finished in 1.1.0 has its checkpoint 17 in a released version's task file, which this reconciliation never opens and which `/hn` never revisits. So each transitive dependent gets **a fresh entry in the paying version's `_plan.md`, under `## Features` with a `[ ]` box, and its own file in `.hora/tasks/<paying version>/<id>.md`** — checkpoints 1–16 marked not applicable against a stated reason, 18 left `[ ]`, and its `Rests on:` line carried across.

```markdown
- [x] 1. Confirm the feature is buildable  <!-- n/a: accepted in 1.1.0; re-accepted because #billing's debt was paid -->
...
**A `/hn-hotfix` run put code on `main` without an acceptance review** (`../hn-hotfix/SKILL.md`). Its record says which features it touched. **Turn that into work the ordinary gates already handle.**

```
for each id on the record's touches: line
    the id has an entry in this version's _plan.md  -> clear its checkpoint 17
                                                       back to [ ], AND its
...
**Both boxes come off together.** A `_plan.md` entry left `[x]` over a feature file holding an open checkpoint 17 is a feature `/hn` step 5 will never pick up, so the debt would never be collected.

**Only checkpoint 17 is cleared, and only in the version being planned.** The hotfix changed code that was already accepted, so what has gone stale is the acceptance, not the build. **No released version's `_plan.md` is ever rewritten** (section 1).
...
every question written to the question file — its Q<n> id, its category, its
  ...
  (../hn/references/structure.md, "Citing a question in a report")
...
**Seventeen here and twenty in the acceptance record are not a discrepancy.** This report counts what there is to build; the sweep's verdict counts what the tag claims about the product, so it keeps all twenty in its denominator (`../hn-accept/SKILL.md`, "Recording the result"). Two questions, two numbers.
...
| File | Content |
|---|---|
| `../hn/references/structure.md` | the layout, the invariants, the language rule |
| `../hn/references/asking.md` | **a check, a proposal or a question** — and the question tool this skill defaults to |
| `../hn/references/spec-format.md` | the authority on the format of `specs/<version>/spec.md` |
| `../hn-spec/SKILL.md` | **who writes a spec, and what to hand back to it** |
| `../hn-spec/references/stages.md` | which stage a design-level finding goes back to |
| `specs/skeleton/spec.md` | the blank spec that gets copied. `/hn-spec` does the copying |
| `../hn-build/references/checkpoints.md` | the checkpoint list to write into each feature file |
| `../hn/references/done-criteria.md` | what "done" means for a checkpoint, a feature and a version |
