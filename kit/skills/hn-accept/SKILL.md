---
name: hn-accept
description: Run acceptance and record what passed and what did not — the full unit suites every time, plus an acceptance review at the reach the invocation calls for. Invoked as checkpoint 17 of /hn-build, as the whole-version sweep, or directly as /hn-accept.
---
...
Read `../hn/references/structure.md` first. **This skill is strictly read-only on `specs/`, and it never fixes code.** It finds and records; fixing is a checkpoint's job, in `/hn-build`.
...
**A skill's name belongs to the package, which is free to change it** (`../hn/references/structure.md`, "No hora file ever names one of those skills"). A renamed skill does not disagree with this file — the name stops matching, and the step is skipped while the record says the run passed.
...
|---|---|---|---|
| checkpoint 17 of `/hn-build` or `/hn-fast` — the feature gate | every repository, in full | **the feature at the gate.** The live, browser-driven part of the review is **skipped unless explicitly requested, or unless this run is paying a listed feature's deferred acceptance** (below) | `.hora/acceptance/<version>/<feature-id>.md` — **a new block, whatever reach this run took** |
| the whole-version sweep — `_plan.md`'s `## Acceptance` entry | every repository, in full | **every done feature** (its live, browser-driven part **asked for at step 1**, every time) — for every version in ascending order, every feature whose entry in `_plan.md` is `[x]`, plus the one at the gate if any, plus — in a version the plan collapsed to one adoption sweep — every entry in that version's feature section whatever its box reads (below) | `.hora/acceptance/<version>/_sweep.md` — a new block |

**Step 6 is the one step whose scope is not a feature set.** At the sweep it is pointed at the repository whole; at a gate it does not run, because checkpoint 7 already audited that feature's change set (`../hn-build/references/checkpoints.md`, checkpoint 7).

**"Explicitly requested" means a person asked for it, in the run.** That is the only widening there is, and the run records it with the requester named. **Nothing here upgrades a gate run on its own judgment**, and the only thing that narrows the sweep is a person declining its live part when step 1 asks.

**No run drives the product without a person having said so**, except the one paying a listed feature's deferred acceptance. Building the stack is slow, and a development loop that pays for it at every feature gate stops being a loop — so checkpoint 16 writes the environment down and starts nothing (`../hn-build/references/checkpoints.md`, checkpoint 16).

**Declining costs the version's closure, never the run.** A declined sweep still runs the unit suites and every static check over every feature in scope; what it gives up is `reach: full`, which is the one thing `/hn` step 7 reads before it merges. So the answer to "not now" is a version that stays open until somebody says yes — not a version that closes on a weaker claim.
...
**The version's own acceptance criteria are a third thing in scope, and they are the sweep's alone.** A behavior spanning several features reaches no gate at all (`../hn/references/spec-format.md`, "15. Version acceptance criteria"). `_plan.md`'s `## Acceptance` entry names how many there are and which rest on a feature nobody accepted.

| Invoked as | The version's own criteria |
|---|---|
| checkpoint 17 of `/hn-build` or `/hn-fast` — the feature gate | **not in scope, at any reach** |
...
**A standing policy could not live in `_plan.md`, which is why the widening is a person and not a line.** `.hora/` holds derivations, written by skills and read by humans (`../hn/references/structure.md`, invariant 1). **If a project wants "sweep live at every gate" as policy, it belongs in the spec.**
...
| A gate run | runs, as its checkpoint 17 | **there is no gate** — 18 stays `[ ]` and nothing marks it |
...
```
1. Confirm the environment — only when this run is going to drive the product
     the skills covering the local end-to-end container stack
     ASK FIRST, at the sweep. Say what bringing the stack up will do and
     what the live part adds, and wait. Declined -> run every other step
     over every feature in scope and record the block `reach: scoped`, with
     the capability note saying the live part was declined and by whom.
     Nothing else shrinks, and the version stays open: closing it reads
     `reach: full` (../hn/SKILL.md, step 7)
     Once it is agreed: the application must run together with every service
     behind it, each role must be able to sign in, and there must be
     reviewable data or a command that produces it.
     ...
     listed feature's deferred acceptance drives, so it needs the stack, and
     it is the one live run that is not asked for: it is the only acceptance
     that code will ever have had
...
2. Unit suites, per repository, from inside it — EVERY run, at EVERY reach
     ...
     AT A FEATURE GATE the backend runs every order-independent test in
     full, plus only the DB-writing categories this feature touched. At the
     SWEEP it runs everything, both kinds. A frontend row has no such split
     and runs whole at every reach ("The two tiers", below)
...
6. Security audit — at the sweep, or on explicit request; a gate run skips it
     ...
     Checkpoint 7 already ran this per feature, over that feature's change
     ...
     skips it; checkpoint 7 covered that feature already, scoped. Findings
...
**The split is structural, not a guess about which tests could have broken.** The backend already files every test by one question — does it write to the database? — and the answer decides the directory (the skills covering backend test placement own it). Order-independent tests need no database and run in parallel; DB-writing ones need a rebuilt, seeded database and run serially. **The cost is almost entirely the second kind.**

| | Order-independent tests | DB-writing tests |
|---|---|---|
| a feature gate | **every one, in full** | **only the categories this feature touched** |
| the sweep | **every one, in full** | **every one, in full** |

**No test is selected by inference.** A whole directory runs, or a whole category does. Nothing reads a dependency graph and nothing decides that a test "cannot" have been affected.

**What a gate gives up is named**: a regression visible only through a DB-writing category this feature did not touch. It is caught at the sweep, and the gate's record says the tier it ran at.

**The sweep never runs at the gate tier**, whatever the invocation. A version's verdict rests on every suite executed for real in that run.
...
1. #attendance — a record saved from the monthly screen is not reachable
   from the daily list. Sends back to: #attendance checkpoint 10.
2. #sign-in — an expired session shows a blank screen instead of saying so.
   Sends back to: #sign-in checkpoint 12.
...
**The reach half is what stops a run that covered part of a version from reading as a whole-version pass.** A widened gate run mid-version has eight features in scope, `reach: scoped`, `not-accepted: none` — under a grammar keyed on the listing alone it would write a bare `passed`. **Two separate things stop that**: the path keeps it out of `_sweep.md`, and `../hn/references/done-criteria.md` reads `reach:` beside the verdict.
...
**`<m>` counts the listed features too.** Seventeen specified and three listed is `17 of 20` — never `17 of 17`, which reads as a run that covered everything there was to cover. **`/hn-plan`'s own closing report counts the other way, on purpose**: a plan of twenty with three listed is seventeen to build (`../hn-plan/SKILL.md`, "When this skill finishes"). **The two are not the same number, and neither is wrong.**
...
**This skill never fixes anything.** It reports, and `/hn-build` acts.

| Kind of finding | What happens |
|---|---|
| the implementation falls short | the named checkpoints are cleared in the named features, and rebuilt through a `retake/` branch (`../hn/references/commits.md`) — the `feature/` branch has already merged by this point |
...
| the spec cannot be satisfied under any reading | a `contradiction` question (`blocking: yes`), and the spec is changed through **`/hn-spec`**, at the stage `../hn-spec/references/stages.md` names |
...
| **a version criterion failed in the part it `rests on:`** — the behavior of a feature the spec only listed | **there is no checkpoint to clear**, so the finding names the debt: pay it in this version, or change the criterion through `/hn-spec`. **Both readings recorded, neither recommended**, as a `contradiction` question (`blocking: yes`) |
| a real finding the project decides to live with | an `acceptance-finding` question, recording the decision and who made it |
| **a finding in a conflict-proof file or in a dependency**, which no feature's checkpoint owns | its own `update/` branch (`../hn/references/commits.md`) — planned growth of something shared, not a redo of a feature. **The run records which branch it named** |

**Two rows name a destination that is not a checkpoint, and both have to.** The rested-on row, because a listed feature has none — re-scheduling the feature to make one would hand code already serving users to `/hn-build` from checkpoint 1. The shared-code row, because a conflict-proof file and a dependency belong to every feature and to none, so clearing one feature's checkpoint would name the wrong owner.
...
**Every question this run raised is reported by name, with a link to the file** (`../hn/references/structure.md`, "Citing a question in a report"). **Never a count.**
...
**The last two are copies checked against their sources, and that is the most a run can check here.** Whether the plan itself matches the spec is stage 7's (`../hn-spec-review/SKILL.md`, "Run the mechanical pass first"). What this run can catch is the drift it introduces itself.
...
| File | Content |
|---|---|
| `../hn/references/structure.md` | the layout, the invariants, the division of labor, how a skill is named |
| `../hn/references/spec-format.md` | `<!-- baseline: inventoried -->`, and **"15. Version acceptance criteria"** — what only a sweep checks |
| `../hn/references/done-criteria.md` | what "done" means for a checkpoint, a feature and a version |
| `../hn-build/references/checkpoints.md` | checkpoint 17, and the checkpoints a finding sends the run back to |
