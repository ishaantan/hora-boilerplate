---
name: hn-hotfix
description: Fix one urgent defect and ship it through six gates, giving up the acceptance review but writing down everything it gave up. Invoked directly as /hn-hotfix when a live product is broken and the normal /hn route is too slow.
---
...
Read `../hn/references/structure.md` and `../hn/references/commits.md` first. **This skill is read-only on `specs/`.** It reports a spec problem; it never fixes one.
...
`/hn-build` takes one **feature** through seventeen checkpoints. This skill takes one **defect** through six gates. **It is not a short version of `/hn-build`.** The unit of work is different, so the gates are different.

| | |
|---|---|
| **What it gives up** | reach — the acceptance review, the browser run, the UX audit, the scenario list, the version's own criteria, `/hn-spec` and `/hn-plan` |
| **What it never gives up** | one failing test, the full unit suites, lint on the files it touched, the git rules, the record |
| **Where the skipped work goes** | `.hora/hotfix/<hotfix-id>.md`, as a written debt. `/hn-plan` turns it back into real work later ("Paying the debt back", below) |
| **What it changes in the normal route** | nothing. `/hn-accept` and the seventeen checkpoints are untouched, so a version's own acceptance is exactly as strict as before |

**This is a lever, and it follows the rule every lever follows** (`../hn/references/structure.md`, "Where a lever lives"). It reduces work. **It may not reduce verification, and it may not reduce what is recorded.** A run that gave up a step pays for it in the record.
...
| **H6** | Land | the main session | `hotfix/<hotfix-id>` is merged into `main`, and the catch-up is handed to `/hn` |
...
work that does not fit one branch, so you want to cut a branch from
    hotfix/<hotfix-id>   (../hn/references/commits.md)
```

### When one of them holds, do not refuse. Show the choices

**A refusal helps nobody at three in the morning.** Work out a real alternative for **this** defect first, then put the choices to the person (`../hn/references/asking.md` — this is a proposal, not a check).
...
2. Do it as a patch-bumped release/<version>
     The normal /hn route. Slower, but it gets accepted
...
**Never work this out yourself.** It states what the product should do, which only a person may state (`../hn/references/structure.md`, invariant 2). That sentence is this run's whole acceptance criteria.
...
Match the equipped skills covering the code being changed, and hand the work to an implementer agent. Run lint from inside that repository, on the files it touched (`../hn/references/structure.md`, "Where a per-repository command runs").
...
**`/hn-accept` owns the words `passed`, `reach:` and `passed over <n> of <m> features; <k> not accepted`,** and readers match those strings (`../hn-accept/SKILL.md`, "Recording the result"). **A hotfix record uses none of them.** It says `landed`, and nothing downstream can mistake it for an acceptance.

### `touches:` names feature ids, and it is checked with a person

Which features the changed files belong to is a label, so it may be worked out from the code (`../hn/references/structure.md`, invariant 2). **Put the answer up as a check before writing it.** It decides which checkpoints get reopened later, so a wrong one loses the debt.
...
6. Hand the catch-up to /hn, which rebases any open release/<version>
   onto the new main (../hn/references/commits.md, "Keeping
...
```
/hn, step 0        reports every .hora/hotfix/*.md whose debt: reads open,
                     by name and with a link

the next /hn-plan  for each id on the touches: line, if that feature has an
                     entry in this version's _plan.md, clear its checkpoint 17
...
Once checkpoint 17 is `[ ]` again, the ordinary route takes over: `/hn-build` picks the feature up, `/hn-accept` accepts it at its real reach, and the version cannot be done until it passes.
...
**This is the one question not answered by editing `specs/`.** It is closed by `/hn-plan` writing `debt: closed` in the record, which is also the run that reopens the acceptance. **Where the record carries a `schema-contract-debt:` line, the question stays open until that work is specified too.**

---

## Matching the skills, and reporting

**Name no package skill anywhere in this file, and none may be added** (`../hn/references/structure.md`, "No hora file ever names one of those skills"). State the work, match the equipped descriptions at run time, and write the names you matched into the record.
...
| File | Content |
|---|---|
| `../hn/references/structure.md` | the layout, the invariants, the division of labor, where a lever lives |
| `../hn/references/commits.md` | `hotfix/<hotfix-id>`, and how an open `release/<version>` catches up |
| `../hn/references/asking.md` | a check, a proposal, a question, and the question tool |
| `../hn-accept/SKILL.md` | the verdict grammar this skill must not use |
| `../hn-plan/SKILL.md` | the question categories, and where the debt is paid back |
