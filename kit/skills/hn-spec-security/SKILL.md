---
name: hn-spec-security
description: Stage 6 of /hn-spec. Give every operation and every screen a stated caller and a stated refusal, name every piece of personal or regulated data, and record why the role-or-endpoint split went the way it did. The one gate with no not-applicable case. Invoked by /hn-spec, or directly.
---

# hn-spec-security

**Stage 6 of `/hn-spec`.** Answer two questions about everything stages 4 and 5 designed: **who may reach this, and what happens when somebody else tries.**
...
| whether the built code has a defect | **the package's audit skills**, at checkpoint 7. They audit code; this stage writes what that audit checks against |
| a token lifetime, a hashing algorithm, a header | the package's own skills, at checkpoint 5 |
...
**Show each addition in full and wait for approval** (`../hn-spec/SKILL.md`).
...
**This is the point of doing security at the spec stage rather than only auditing code.** A refusal with an acceptance criterion is tested at checkpoint 5; a refusal that exists only in prose is checked by whoever happens to think of it.

---

## Exit condition

Every operation naming its caller and its refusal; every screen naming its actor; every personal or regulated field named; the endpoint split's reason written. `../hn-spec/references/stages.md` is the authority.
...
| `../hn-spec/references/principles.md` | "Authorization is the thing left unsaid", and the roles-or-endpoints table |
| `../hn-build/references/checkpoints.md` | checkpoint 7, the audit that checks code against what this stage wrote |
| `../hn/references/spec-format.md` | where a caller and a criterion each belong |
