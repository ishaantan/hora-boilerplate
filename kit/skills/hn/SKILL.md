---
name: hn
description: Implement an application from its spec. Decides where a project stands and runs the five skills that do the work — spec, setup, plan, build, accept — building two features at a time and reporting once per landed pair. Runs at the root of the hora repository. Started or restarted only by an explicit `/hn` invocation.
---
...
`/hn` does no implementation of its own. Five skills do the work:

| Skill | Does | Runs |
|---|---|---|
| **`/hn-spec`** | writes the version's spec, in conversation — stage 0 reads what already exists, then its own seven stages | once per version, until stage 7 passes |
| **`/hn-setup`** | creates the repositories the spec declares, fills in the project's values, reads the real tree | once per version, idempotent |
| **`/hn-plan`** | fixes the version, verifies the spec in conversation, writes the feature list | once per version, re-entered every run |
| **`/hn-build`** | takes one feature through the seventeen checkpoints | **once per feature** |
| **`/hn-accept`** | runs the full unit suites every time, and the acceptance review at the invocation's reach | at each feature's checkpoint 17 (scoped), and once as a whole-version sweep (full) |

**An emergency fix is not one of the five.** `/hn-hotfix` takes one defect to `main` on its own, gives up the acceptance review, and records what it gave up as a debt. `/hn` reports that debt; `/hn-plan` turns it back into work (`../hn-hotfix/SKILL.md`).
...
**`/hn-fast` is the default scheduler, and `/hn` hands the build phase to it.** It builds features two at a time, each in its own git worktree, on the same feature files this one writes, and reports once per landed pair (`../hn-fast/SKILL.md`, "Reporting cadence").

**`/hn` still owns everything before and after the build phase** — the equipment check, step 0's fetch, `/hn-spec`, `/hn-setup`, `/hn-plan`, the merges and the sweep. What it hands over is the order features are built in, nothing else.

| | |
|---|---|
| two or more features still `[ ]` | **hand the build phase to `/hn-fast`.** No question is asked; say which pair opened |
| one feature left | build it here, serially. A lane with no partner buys nothing |
| a person asked for one at a time | build serially, and record that they asked |

**Inside a checkpoint, its units do run together.** Five of the seventeen divide into units whose files are exclusive — a table, a module, an operation, a component, a screen — and each gets an implementer of its own. The checkpoint stays one gate with one exit condition (`../hn-build/SKILL.md`, "Step 5 — splitting a checkpoint into units").

---

## Whether hora can start at all

**The procedures `/hn` orders are not in this repository**, and `/hn` never puts them in place itself. They arrive as skills of their own, alongside these, and this is the one thing to confirm before deciding anything else.
...
Is /hn-setup under .claude/skills/ too?
                             if not → stop. Report that code setup is not
                                      equipped, and ask for it before
                                      rerunning
```

**`/hn-setup` is asked for here because it is not in this package either.** What it does is one stack from beginning to end — which repositories exist, what fills them, what to read once they arrived — so it is written by the boilerplate that knows that stack, and equipped from there. `/hn` orders it like the rest, and a project whose boilerplate has not put it in place cannot get past step 2.

**Nothing here checks the hora skills themselves.** `/hn` is one of them, so a session that reached this line has already proved they are equipped. What cannot be known from the inside is the other package's state, and that is the whole of what this section asks.
...
0. git fetch origin --prune, for the hora repository and every declared row
   ...
   .worktrees/ is a feature /hn-fast has in flight. Stop, and say to drain
   it through /hn-fast (../hn-fast/SKILL.md, "Switching between the two
   schedulers")

1. Does the target version have a spec at all — a specs/<version>/spec.md with
   content in it?                             if not → /hn-spec.
                                              A version with no spec declares no
                                              layout, so /hn-setup has nothing
                                              to read and /hn-plan nothing to
                                              verify

2. Are all declared repositories present, per the current spec's layout?
                                              if any is missing  → /hn-setup

3. Always run /hn-plan. It fixes the version, and reconciles the feature
...
6. No [ ] entry remains that the ## Acceptance sweep entry does not close, and
   ...
   declared                                   → /hn-accept, whole-version

6a. It does, and entries that sweep closes still stand [ ]
                                              → /hn-plan again, to set them
                                                and their checkpoint 17 off
                                                that block (../hn-plan/
...
**Step 0 is also what catches `release/<version>` up with a `hotfix/*`.** `/hn` has no scheduler: this fetch and the one right after every merge into `release/<version>` are the only two occasions it gets to notice one landed on `main`.

**An open hotfix debt is reported here, never acted on here.** Step 3's `/hn-plan` is what turns it back into work (`../hn-hotfix/SKILL.md`, "Paying the debt back").

**`/hn` does not ask before running this check, or before acting on an ordinary result.** It stops and asks once the check turns up something it cannot resolve on its own.

**Step 1 is not "write the spec for them".** `/hn-spec` writes nothing without somebody reading it first, and a run that reaches step 1 with nobody there to answer stops there.

**Step 3 runs even when the feature list already exists.** A spec keeps moving while implementation is under way. Only once `/hn-plan`'s reconciliation shows no difference does a version move on.
...
**Step 5 also passes over an entry whose checkpoint 17 the `## Acceptance` sweep entry covers.** Its `[ ]` means *the sweep will close this*, not *hand this to `/hn-build`* (`../hn-plan/SKILL.md`, "collapses to one sweep"; `../hn-build/SKILL.md`, "Where to start").
...
**Keyed that way the two steps leave no gap.** Whatever fails step 7's test — no record, `reach: scoped`, a `failed` verdict, a short `version-criteria:` line — satisfies step 6's predicate, so the run goes back to `/hn-accept`.

**Step 6a exists because the run that earns a collapsed version's checkboxes must also set them.** `/hn-accept` records and never writes `_plan.md`; `/hn-build` never opens those entries, which is the collapse; so `/hn-plan` is the writer, off the sweep's own record. Left to the next invocation's step 3, the merge happens first — and a released version's plan is never rewritten (`../hn-plan/SKILL.md`, "Resolve the diffs first").
...
Report the decision in one line before starting work — for example, "continuing 1.0.0. 4 of 11 features done, building #payroll from checkpoint 5".

---

## What `/hn` owns, and what it never does

**Every git operation happens in the main session** — whether `/hn` runs it directly or a skill it invoked does (`/hn-setup` initializing a row, `/hn-build` cutting a feature branch). **No agent any skill starts ever touches git.**

| | Who does it |
|---|---|
| git, in every repository | **the main session** — `/hn` and the skills it runs. Never an agent |
| writing `.hora/` | the skill whose work it records (`/hn-plan` the plan, `/hn-build` the checkpoints, `/hn-accept` the acceptance records) |
| writing `specs/` | **`/hn-spec`, one approved stage at a time, and `/hn-plan`, one approved batch of edits at a time. Nobody else** (`references/structure.md`, invariant 1) |
| writing code and tests | the agents `/hn-build` starts |

**Manual verification is not one of the phases.** A human does it whenever they want, in the backend row, with the commands `/hn-setup` recorded in `.hora/tree/<repository>.md`. What *is* required, whenever a run drives a browser, is the local end-to-end environment checkpoint 16 declares: `/hn-accept` stops without it. **Nothing brings that stack up during development** — checkpoint 16 writes it down and starts nothing, and `/hn-accept` starts it only for a run that is about to drive the product, asking first at the sweep (`../hn-accept/SKILL.md`, "What is in scope").

---

## Where the procedures live

**`/hn` holds the order. It holds no procedure and no pass/fail criterion.** How to write a resolver, a migration, a component or a test — and what an acceptance review looks at — all live in the `@openreachtech/hora-skills-ort-*` packages, equipped under this repository's own `.claude/skills/`.
...
what the next run of /hn will start from
```
...
**Every feature the version listed rather than accepted is named by id too, and never counted.** Name each one, and name what rests on it: a dependent records `Rests on: #x (not accepted)` in its own feature file (`../hn-plan/SKILL.md`, "One file per feature").

**This report is where such a debt is most easily lost.** The verdict and the findings are what everybody reads, and a closing report that ends at "passed" is the sentence somebody remembers a month later (`../hn-accept/SKILL.md`, "Recording the result").
...
Remaining: #payroll #bonus #year-end
  ...
  split it        → cut the release at the last accepted feature: kicked: yes
                    ...
                    version through /hn-spec
                    (/hn-spec-horizon, "Splitting a version under way")
```
...
**`/hn` only lays out the choices; it does not decide.**
...
| `../hn-spec/SKILL.md` | **the author** — how a version's spec gets written |
| `../hn-spec/references/stages.md` | stage 0, then the seven stages, and each one's exit condition |
| `../hn-spec/references/investigation.md` | what stage 0 reads, and the line between a fact and an intent |
| `../hn-spec/references/principles.md` | the thinking a spec is written with |
| `/hn-setup`, as the project's boilerplate ships it | code setup |
| `../hn-plan/SKILL.md` | the planner |
| `../hn-build/SKILL.md` | one feature through the checkpoints |
| `../hn-build/references/checkpoints.md` | the seventeen checkpoints themselves |
| `../hn-accept/SKILL.md` | acceptance |
| `../hn-hotfix/SKILL.md` | one urgent defect to `main`, the six gates it runs, and the debt that pays for the acceptance it skipped |

**When a human asks how to write a spec, run `/hn-spec`.** `specs/1.0.0/spec.md` ships empty, and that skill reads whatever already exists at stage 0, copies the skeleton, asks its way through seven stages, and writes each section once it has been approved.
