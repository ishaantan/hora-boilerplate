...
`/hn`, `/hn-fast`, `/hn-spec` (and its seven stage skills), `/hn-setup`, `/hn-plan`, `/hn-build`, `/hn-accept` and `/hn-hotfix` all stand on this file. **It is written once here and read by all of them** — a copy in a skill is what goes stale.
...
|---|---|---|
| which phase runs next, and when a project is ready for it | Hora Kit | `/hn` |
| **the order a spec is decided in, and what must be settled before the next thing** | Hora Kit | `/hn-spec` |
| which repositories exist, and what fills them | Hora Kit | `/hn-setup` |
| which version is being built, and which features it holds | Hora Kit | `/hn-plan` |
| **the order of the checkpoints, and each one's exit condition** | Hora Kit | `/hn-build` |
...
| **which boilerplate fills a declared row, what gets filled in, and what to read once it arrived** | **the stack handbook** | `docs/stack/` at the project root, shipped by the boilerplate (the stack handbook reference of `/hn-setup`, which the boilerplate ships) |
...
**Skills Hora Kit itself ships may be named freely** — `/hn-spec`, `/hn-plan`, `/hn-build`, `/hn-accept`, `/hn-hotfix`, `/hn-fast`, `hn-implementer`, `hn-verifier`, `hn-digester`. They live in this repository, so a rename here is a rename everywhere.

### How the match is made

**What sits under this repository's own `.claude/skills/` is what can be matched** — whatever is there at the time, however it was put there. Those are invocable through the ordinary `Skill` tool. A run entered through `/hn` stops before reaching any of this when nothing is equipped at all (`../SKILL.md`, "Whether hora can start at all"). A skill invoked directly does not, and matches against whatever it finds.
...
4. It hands those names to the agent that runs the work, each with a digest
   of that skill (../../hn-build/SKILL.md, "Step 3 — the digest each
...
|---|---|---|
| `specs/` | **`/hn-spec`, `/hn-plan`, and humans** | **write, and only with approval: `/hn-spec` a section at a time, `/hn-plan` an edit at a time. Every other skill is read-only** |
...
|---|---|---|
| **`/hn-spec`** | a whole version's spec, from a conversation with whoever wants the product — **and, from the second version on, that version's diff** | **a section**, at the end of the stage that drafted it |
| **`/hn-plan`** | the holes and contradictions found while planning | **an edit** |
...
**`specs/skeleton/spec.md` is written to by nobody, and is not a version.** It is the blank spec copied to `specs/1.0.0/spec.md`, **for the first version only** (`spec-format.md`). `/hn` reads only the directories under `specs/` whose name is a semver version, so the skeleton is never planned, implemented, or counted as unfinished.

**Every other skill — `/hn-setup`, `/hn-build`, `/hn-accept`, `/hn-hotfix`, and every agent — is strictly read-only on `specs/`.** They report a problem there; they never fix it. A typo and a broken layout are treated the same.

**`/hn-build`'s checkpoint 1 is where the routing happens, not an exception.** A design hole goes to `/hn-spec` at the stage that owns it, a one-line hole to `/hn-plan`'s propose-and-approve procedure (`../../hn-build/references/checkpoints.md`, checkpoint 1).
...
| **A permanent identifier** | **`id`** | **Must not be invented.** Derive it only where it can be derived (`/hn-plan`) |
...
**Do not reduce what is asked. Reduce how often the run interrupts** — a stage's questions go out in one batch, a feature is put to a person at most three times, and anything the skill can defensibly draft is drafted as a proposal rather than asked as a blank question (`asking.md`, "A feature's budget is three turns" and "Draft it, then ask what is wrong with the draft").
...
.hora/
  ...
  tree/<repository>.md          what /hn-setup read in the real tree, and the tag it read it at
  digests/<skill-name>.md       one equipped skill's conventions in short form, and the
                                content hash of kit/skills/<skill-name>/ they were
                                derived at. hn-digester writes it. A cache; the skill
                                stays the authority
  tasks/<version>/
    _plan.md                    the feature order, and the acceptance tasks. /hn-plan writes it
    <feature-id>.md             one feature. /hn-plan creates it, checklist and all;
                                /hn-build writes the checkboxes, the matched skills and each
    ...
    _fast.md                    the parallel scheduler's record: who chose it, the limit, and the
                                foundation with its run record. /hn-fast writes it
  ...
  hotfix/<hotfix-id>.md         one urgent fix that went straight to main: what it changed,
                                ...
                                /hn-hotfix writes it; /hn-plan closes it
