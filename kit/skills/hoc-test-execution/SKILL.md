...
description: >
  ...
  which suites a given run executes, to the acceptance convention.
---
...
**Running a narrower set of suites is not one of these.** A run that executes every order-independent
test and only the DB-writing categories a feature touched has skipped no test and loosened none — it
executed a stated set, in full, and its record names the set. Which set a given run owes belongs to
the acceptance convention, never to this one. **What this file forbids is a suite that was run and
then made to look greener than it was.**
