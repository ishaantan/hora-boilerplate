...
Read `../hn/references/structure.md` first. **`../hn/references/asking.md` fixes how anything is put to a person** — a check, a proposal or a question. **`references/principles.md` holds the thinking this skill applies**, **`references/investigation.md` what stage 0 reads**, and **`references/stages.md` is the authority on stage 0 and the seven stages.**

`/hn-spec` does no design of its own. **Stage 0 it runs itself; seven skills do the rest, in order:**

| Stage | Skill | Fixes |
|---|---|---|
| **0** | **— (this skill)** | **what already exists** — the repositories, the documents, and what they show |
| 1 | **`/hn-spec-usecases`** | who uses this, and what each of them completes end to end |
| 2 | **`/hn-spec-horizon`** | what this release builds, what is foreseen for later, what is never built |
| 3 | **`/hn-spec-nonfunctional`** | how many users, how heavy, how available, how long kept |
| 4 | **`/hn-spec-backend`** | the repositories and servers, the data model, the operations, what runs as a job |
| 5 | **`/hn-spec-frontend`** | the screens each use case passes through, and what each screen calls |
| 6 | **`/hn-spec-security`** | who may call each operation, and what happens when someone else does |
| 7 | **`/hn-spec-review`** | whether the whole document holds together, and every use case is satisfiable |

---

## Why this skill exists

**A blank spec plus a format document is a writing assignment.** The format is exacting — use cases and acceptance criteria per feature, the kind of every operation, two different kinds of out-of-scope, an `id` that may never change — so a person handed it writes the parts they find easy and leaves `/hn-plan` to ask about the rest, one question at a time.
...
| a requirement **nobody stated and nobody approved** | **never written** (`../hn/references/structure.md`, invariant 2) |

**The check and the two kinds of proposal are different acts and must never sound alike.** A check asks whether the skill read the system correctly; a proposal asks whether to do something the system does not do. Stated in the same voice, a proposal becomes an existing fact. **`../hn/references/asking.md` is the authority.**
...
**Step 3 stays in prose, never in the question tool.** An option labelled "approve" is precisely what lets somebody not read the words (`../hn/references/asking.md`). The individual checks and proposals inside the conversation default to the tool, with the likely answer offered first.
...
**`/hn-plan`'s per-edit rule is unchanged, and the two do not compete.** A one-line hole found while planning is settled there. **A finding that needs design work comes back here**, to the stage that owns it (`references/stages.md`, "What sends a run back into a stage").
...
| | |
|---|---|
| `specs/<version>/spec.md`, and the version's feature files | **written by this skill**, one approved stage at a time — every section that stage owns goes up together, and none of them is written until the stage is approved |
...
| `.hora/spec/<version>/_stages.md` | this skill's own record of where it got to. **One that already exists is added to, never replaced** — a split of the previous version may have left a handoff in it (`../hn-spec-horizon/SKILL.md`, "Splitting a version under way") |
...
| `.hora/tasks/`, `.hora/contracts/`, `.hora/glossary.md` | **never.** They are `/hn-plan`'s |
| code, tests, any implementation repository | **read, never written.** Nothing read becomes a requirement on its own |
| git, in any repository | **never.** `/hn` owns every git operation |
...
1. The version is the one given on the command line, or:
   ...
   /hn-plan would target (../hn-plan/SKILL.md, "Fix the version")
...
2. If specs/<version>/ does not exist, create it, with sources/, annex/ and
   ...
   being committed (../hn/references/spec-format.md, "a drop-off convention")
...
4. From the second version on, DO NOT copy the skeleton. What gets written is a
   ...
   (../hn/references/spec-format.md, "The blank spec is not copied into a
   diff version")
...
**Nothing about the format changes. What changes is how much of it this version writes** (`../hn/references/spec-format.md`, "From the second version on, write a diff").
...
**Drop what is wanted into `specs/<version>/request/` and run `/hn-spec`.** Any file, any name, in anybody's own words. Stage 0 reads it first and treats it as **this version's agenda** (`references/investigation.md`).

**A request is not a source, and the difference is the point.** Nothing in it is spec text. What it says clearly is drafted into the section that owns it and goes back as a **proposal**; what it implies goes back as a question. `/hn-plan` never reads `request/` at all, so a wish list cannot become a task by sitting in a folder.
...
Appended to `.hora/questions/<version>/open.md`, in the format and the language `../hn/references/structure.md` fixes. Three categories exist for this stage, and `../hn-plan/SKILL.md` holds the full table:
...
**A `blocking: yes` question does not stop this skill from finishing the other stages.** It stops `/hn-build`. Carry on to the end of stage 7 with the hole recorded.
...
what /hn will start on next (normally /hn-setup, then /hn-plan)
```
...
**Never report questions as a count** (`../hn/references/structure.md`, "Citing a question in a report").
...
- **clone or configure a repository.** Declaring the layout is stage 4's; creating it is `/hn-setup`'s
...
| `../hn/references/asking.md` | **the authority on how anything is put to a person** |
...
| `../hn/references/spec-format.md` | **the authority on the format** of what this skill writes |
| `specs/skeleton/spec.md` | the blank spec this skill copies |
| `../hn-plan/SKILL.md` | what happens to the spec next, and the question categories |
| `../hn-spec-usecases/SKILL.md` … `../hn-spec-review/SKILL.md` | the seven stages themselves |
