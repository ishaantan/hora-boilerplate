...
**The authority on the stage list.** `/hn-spec` copies the list from here into `.hora/spec/<version>/_stages.md`; each stage skill runs one of them.
...
**This file holds the order and the exit conditions. It holds no design rule.** How a table is shaped, how an API schema is named, where a background job belongs and what a screen must account for all live in the `@openreachtech/hora-skills-ort-*` packages (`../../hn/references/structure.md`, "The division of labor").

**No stage below names a package skill, and none ever may.** Each **Delegate to** row says what has to be covered, and the main session matches that against the equipped skills' own descriptions when it enters the stage (`../../hn/references/structure.md`, "No hora file ever names one of those skills").
...
**Which form each thing goes out as is not a stylistic choice** — a fact read off the system is a check, a gap the stage noticed is a proposal, and something nobody has decided is a question (`../../hn/references/asking.md`).
...
**Draft them; never demand them blank.** Whoever wants the product rarely arrives with observable criteria, so the stage derives them from the use cases and puts the draft out **as a proposal** (`../../hn/references/asking.md`). A person handed an empty block writes none, and the missing block is `blocking: yes` at stage 7.

**A block holds only what its own feature's gate can check, and what reaches further belongs to stage 2** (`../../hn/references/spec-format.md`, "A criterion is checked at its own feature's gate"). **The stage that drafted it records it and hands it over; it never places it and never drops it**, because placing it needs the order, and the order is stage 2's. **This binds every stage that writes a block** — 1, 4 and 5 alike.
...
**`/hn-plan` and `/hn-build` use this table too.** A finding at checkpoint 1, 8, 10 or 18 that turns out to be a shortfall in the spec comes back to `/hn-spec`, at the stage this table names.
...
| | |
|---|---|
| **Skill** | **none — `/hn-spec` runs it itself.** It gathers and decides nothing |
...
**Nothing it reads becomes a requirement.** A draft goes out as a check, and only what somebody confirms is written (`../../hn/references/structure.md`, "This forbids inferring. It does not forbid reading").
...
| **Reads** | the operation and screen surface, for the **feature list** it implies and the **actor candidates** the role checks imply. Never for what a feature is *for* — **except under `Authority: as-built`, where the screen-to-operation paths and the existing tests are the material use cases and acceptance criteria are drafted from, as checks** (`../../hn-spec-usecases/SKILL.md`, "Drafting use cases from the running system") |
...
| **Writes** | `Implementation scope`, in three parts, `Implementation plan`, and **`Version acceptance criteria`** — and, in a split of a version under way, the moving sections' `kicked:` lines, on both versions across the handoff (`../../hn-spec-horizon/SKILL.md`, "Splitting a version under way") |
...
**The two kinds of out-of-scope are not a formality.** "For now" makes `/hn` leave an extension point; "permanently" makes it exclude the thing from the design.
...
**A listed section names nothing to build, and it belongs in none of the three lists.** This version builds and accepts no part of it, there is nothing to unblock, and nothing was excluded from the design. **What this stage does with it is leave it alone** (`../../hn/references/spec-format.md`, "`baseline`").
...
**An operation's kind is never inferred** — `/hn-build` branches on the value at three separate checkpoints. Ask, and write what was said.
...
**A listed feature's operations get this stage's full work, undiminished.** Listing suspends exactly two checks, and neither is this stage's (`../../hn/references/spec-format.md`, "`baseline`"). **Anything reachable without authentication is still asked about one operation at a time**, never batched.
...
**Counting them is part of the exit condition**, because the count is what `/hn-plan`'s `## Not accepted` entries and every acceptance verdict's `not-accepted:` line are checked against.
...
| `../../hn/references/asking.md` | **a check, a proposal or a question** — and the question tool every stage defaults to |
...
| `../../hn/references/spec-format.md` | **the authority on the format** every stage writes into |
| `../../hn/references/structure.md` | the invariants, the division of labor, the language rule |
| `../../hn-build/references/checkpoints.md` | the seventeen checkpoints, and the four that can send a run back here |
