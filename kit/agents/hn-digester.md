---
name: hn-digester
description: Digest one equipped skill into the conventions an implementer follows, and write it to .hora/digests/<skill-name>.md. Read-only everywhere else. Called by /hn-build when a matched skill has no digest at that skill's current content hash.
...
the source       kit/skills/<skill-name>/, and its content hash — sha256 over every
                 file in that directory, path and contents, shortened to 12 chars
...
**What makes this safe is that the digest is not the last word.** It names its source, `/hn-build` uses it only while the recorded content hash still matches the skill on disk, and the implementer opens the skill itself the moment a question stays open. A rule you compress too far therefore costs one read. **A rule you drop silently costs a convention**, and that is the one failure worth writing carefully against.

**The hash is what a version number used to be, and it is strictly better here.** These skills are vendored — they carry no package and no release, so there is no version to compare. A content hash changes the moment somebody edits a skill in `kit/skills/`, **including an edit nobody committed**, which is exactly when a digest must stop being read.
...
**The two comment lines are what make the digest usable at all.** The version is what `/hn-build` checks before handing this file to anybody, so a digest without it is one nobody may read; the source path is where the implementer goes when this text runs out.
...
**Say plainly where the digest is thin.** `/hn-build` records that alongside the checkpoint, and a named gap is one somebody can widen later.
