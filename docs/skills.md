...
Two documents describing the same convention will disagree — the question is only when, and whether anybody notices. The package moves a path; the copy in the kit stays there, still confident, now wrong. **A stale copy does not announce itself:** it reads exactly as authoritative as the day it was written, and an agent following it produces work that is confidently in the wrong place.
...
This is the same reasoning `/hn-setup` already applies to the boilerplates: **read the real thing; do not bake in what it currently says.** The package is the real thing here.

### What that leaves each side owning

| | Owns | Example |
|---|---|---|
| **Hora Kit** | when something happens, and what must be true before the next thing may | *"Checkpoint 3 passes when a schema-accurate stub exists for every operation another feature is waiting on"* |
...
Claude Code discovers skills only in the session's own `.claude/skills/`. They are held at `kit/skills/` and `kit/agents/`, which is not that path — so without a copying step, **everything here stays invisible.**

`npm install` runs the copy, through this repository's own `postinstall`:

```json
"hora:init": "node kit/scripts/equip-own-skills.mjs",
"postinstall": "npm run hora:init"
```

```
kit/agents/<agent>.md  ─>  .claude/agents/<agent>.md
kit/skills/<skill>/    ─>  .claude/skills/<skill>/
              straight copy, no renaming, no rewriting
```

- **They are vendored, not installed.** The skills and agents came in from `@openreachtech/hora` and the four `@openreachtech/hora-skills-ort-*` packages, and are now held here as source. Those packages are gone from `devDependencies`: nothing is fetched, nothing is versioned apart from this repository, and the `/hn-*` commands are this repository's to name
- **What that costs is upstream.** A fix released in one of those packages no longer arrives by raising a range. Bringing it across is a deliberate act, against a tree that has since been renamed
- **`npm install` alone is enough**, and `npm run hora:init` re-equips on demand
- **The copy is repeatable.** Both destination directories are emptied before it, so a skill renamed or dropped under `kit/` does not linger as a match candidate
- **The copies are gitignored, and excluded from the root lint.** `kit/` holds the source; everything under `.claude/` is generated, whoever wrote it

One thing under `kit/` is read rather than equipped: **`kit/ecosystem/`**, the vendored `@openreachtech/hora-ecosystem` — the catalog of in-house packages that checkpoint 5 checks before anything is written new. It is copied into no session directory: it is read in place, and its layout is the catalog's own to change ([`checkpoints.md`](../kit/skills/hn-build/references/checkpoints.md), checkpoint 5).

**`kit/eslint/` is vendored the same way** — the shared `@openreachtech/eslint-config` and the three rule packages under it, which the root `eslint.config.js` now imports by relative path. It equips nothing and no skill reads it; it is here so that no `@openreachtech/*` package is resolved from the registry at all.

---

## No hora file names one of these skills

Not [`checkpoints.md`](../kit/skills/hn-build/references/checkpoints.md), not [`stages.md`](../kit/skills/hn-spec/references/stages.md), not an agent definition, not this page. **A skill's name belongs to the package, which is free to change it** — and a written-down name is the one kind of copy that fails silently.

A renamed skill leaves the kit delegating to a name that matches nothing: the gate runs without its convention, and reports that it passed.

**A stale procedure at least disagrees with the real thing the moment somebody reads both. A stale name disagrees with nothing** — it simply stops resolving, and a skipped convention becomes a one-line footnote under a passing run.
...
```markdown
- [x] 15. UI  <!-- skills: <every name matched>; digests: <content hash> -->
...
**A matched skill runs to thousands of lines, and it stays resident for every turn the agent reading it takes.** A checkpoint's cost is close to that resident size multiplied by its turn count, so what `/hn-build` hands an implementer is `.hora/digests/<skill-name>.md` — the same conventions in short form, written by [`hn-digester`](../kit/agents/hn-digester.md) the first time a matched skill has no digest at that skill's current content hash. That is also why the record above names the hash the digests came from, beside the names themselves.

**A digest is a copy, and it is the one copy the rule this page opens with admits** — because it cannot go stale in silence. It carries a sha256 over `kit/skills/<skill-name>/` in its own header, so it is read only while that hash still matches the skill on disk, and editing a skill leaves its digest to be rewritten before it is read again — an uncommitted edit included. Since the skills are vendored and carry no release, the hash is what a version number used to be, and it catches strictly more. It reduces what an agent holds resident, and it decides nothing.

**When the two disagree, the skill's own text settles it.** A digest names the file it came from, and the agent opens that file the moment a question stays open. A convention a digest states too briefly costs one read; it is not a convention lost.

**Nothing whose skill *is* the criteria is read this way.** The security audit at checkpoint 8 and the acceptance review invoke their skills whole, because an agent writing code has a moment where the short form announces its own gap and an audit does not: the missing check is the one nobody thinks to ask about. **A summarized check list is a shorter check list, and it reports a pass.** ([`structure.md`](../kit/skills/hn/references/structure.md), "How the match is made")
...
And the authoritative statement of **what work** each checkpoint delegates is [`checkpoints.md`](../kit/skills/hn-build/references/checkpoints.md); for a spec stage it is [`stages.md`](../kit/skills/hn-spec/references/stages.md). **Neither is repeated here, deliberately** — a second copy of either would be exactly the drift this whole document is about.
...
|---|---|---|
| **writing a stub API** | **3** | it is how one feature's frontend stops waiting on another feature's backend. Applicable only where a dependent is actually waiting, and the stub shares a class name and interface with the real resolver, so checkpoint 15 is a change of endpoint, not a rewrite |
...
| **the acceptance review** | **18** | it holds every criterion acceptance passes or fails on. `/hn-accept` contributes scope, order and a record — nothing else |
...
| `/hn-accept` | records the gap in the run's own record. **A run with a step missing is not a pass with a footnote** — it is a partial run, and the record has to say so |
...
| | |
|---|---|
| the authoritative statement of what work each checkpoint delegates | [`checkpoints.md`](../kit/skills/hn-build/references/checkpoints.md) |
| the boundary, stated as a rule | [`structure.md`](../kit/skills/hn/references/structure.md), "The division of labor" and "No hora file ever names one of those skills" |
