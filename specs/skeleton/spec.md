<!--
  A blank spec. It gets copied to specs/<version>/spec.md and filled in.
      ...
      /hn-spec                        writes it with you, in conversation
  ...
  required, and what makes /hn stop and ask are all in
  .claude/skills/hn/references/spec-format.md. Read that; write this.

  /hn-spec does the copying itself, and writes each section once you have
  read and approved it. Nothing else ever writes into specs/, except
  /hn-plan, one approved edit at a time.

  specs/skeleton/ is not a version. /hn only ever reads directories under
  ...
  renumber freely — /hn reads `id`, never a section number.
  ...
  /hn-spec's stage 0 reads them, confirms the split with you, and fills in
  ...
  ticket, a page of bullets, in your own words — and run /hn-spec. Stage 0
  ...
  becomes spec text on its own, and /hn-plan never reads it.
  ...
  be guessed. /hn-spec confirms it with you, one feature at a time, showing
